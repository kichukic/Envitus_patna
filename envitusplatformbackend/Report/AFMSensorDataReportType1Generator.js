...
function AFMSensorDataReportType1Generator() {
                ...
                if (result != null && result.dailyStat != null) {
                    ...
                    var processDevice = function () {
                        ...
                        if (statPerDay[dailyList[k].key] == null) {
                            ...
                            var dateItems = dailyList[k].key.split(".");
                            statPerDay[dailyList[k].key]._Date = dateItems[2] + "-" + dateItems[1] + "-" + dateItems[0];  //dailyList[k].key;
                        ...
                        if (dailyList[k].paramName == "receivedTime") {
                            ...
                            statPerDay[dailyList[k].key]["PerAgeUpTime"] = downPercent.toFixed(2);;
                        }
                        else if (dailyList[k].paramName == "temperature") {
                            paramName = "Temperature";
                            statInfo = {
                                maxTemp: dailyList[k].statParams.max,
                                minTemp: dailyList[k].statParams.min
                            };

                        }
                        else if (dailyList[k].paramName == "pressure") {
                            paramName = "Pressure";
                            statInfo = {
                                maxPressure: dailyList[k].statParams.max,
                                minPressure: dailyList[k].statParams.min
                            };

                        }
                        else if (dailyList[k].paramName == "humidity") {
                            paramName = "Humidity";
                            statInfo = {
                                maxHumidity: dailyList[k].statParams.max,
                                minHumidity: dailyList[k].statParams.min
                            };
                        }
                        else if (dailyList[k].paramName == "PM2p5") {
                            paramName = "PM2P5";
                            statInfo = {
                                maxPM2P5: dailyList[k].statParams.max,
                                minPM2P5: dailyList[k].statParams.min
                            };
                        }
                        else if (dailyList[k].paramName == "CO2") {
                            paramName = "CO2Level";
                            statInfo = {
                                maxCO2Level: dailyList[k].statParams.max,
                                minCO2Level: dailyList[k].statParams.min
                            };
                        }
                        if (paramName != null) {
                            statPerDay[dailyList[k].key][paramName] = statInfo;
                        }
...
module.exports =
 {
     AFMSensorDataReportType1Generator
 };